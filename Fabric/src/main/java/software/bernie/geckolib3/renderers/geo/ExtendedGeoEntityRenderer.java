...
public abstract class ExtendedGeoEntityRenderer<T extends LivingEntity & IAnimatable> extends GeoEntityRenderer<T> {
	...
	protected void setLimbBoneVisible(GeoArmorRenderer<? extends ArmorItem> armorRenderer, ModelPart limb,
		...
		if (limb == armorModel.head || limb == armorModel.hat) {
			...
			armorRenderer.setBoneVisibility(
					(slot == EquipmentSlot.FEET ? armorRenderer.leftBootBone : armorRenderer.leftLegBone), true);
			...
			armorRenderer.setBoneVisibility(
					(slot == EquipmentSlot.FEET ? armorRenderer.rightBootBone : armorRenderer.rightLegBone), true);
		}
	}
	...
	protected void prepareArmorPositionAndScale(GeoBone bone, List<Cube> cubeList, ModelPart sourceLimb,
		...
		if (!geoArmor) {
			...
			if (modMatrixRot) {
				poseStack.mulPose(new Quaternionf().rotationXYZ(0, 0, (float) Math.toRadians(zRot)));
				poseStack.mulPose(new Quaternionf().rotationXYZ(0, (float) Math.toRadians(yRot), 0));
				poseStack.mulPose(new Quaternionf().rotationXYZ((float) Math.toRadians(xRot), 0, 0));
			} else {
