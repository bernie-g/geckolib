...
/// Handles the state of an animation for a given [GeoAnimatable].
///
/// This is what functionally stops/starts/plays animations for your animatable.
///
/// @param <T> The [GeoAnimatable] type this controller belongs to
...
public class AnimationController<T extends GeoAnimatable> {
    ...
    /// Instantiates a new `AnimationController` with a default name
    ///
    ...
    ///
    /// @param name The name of the controller - should represent what animations it handles
    /// @param transitionTicks The amount of time (in **ticks**) that the controller should take to transition between animations.
    ...
    /// @return The current `AnimationTimeline` instance for the [#currentRawAnimation] playing, or null if no animation is playing
    ...
    /// or [#NOT_ANIMATING] if no animation is playing,
    /// or [#FINISHED_ANIMATING] if the animation has finished playing
    ...
    /// [#triggerableAnim]
    /// Used for custom handling if [#receiveTriggeredAnimations()] was marked
    ...
    /// Adds the provided [KeyframeEventHandler] to this controller, for handling [SoundKeyframeData] keyframe events
    ///
    /// @return this
    ...
    /// Adds the provided [KeyframeEventHandler] to this controller, for handling [ParticleKeyframeData] keyframe events
    ///
    /// @return this
    ...
    /// Adds the provided [KeyframeEventHandler] to this controller, for handling [CustomInstructionKeyframeData] keyframe events
    ///
    /// @return this
    ...
    ///
    /// **<u>NOTE:</u>** This value does not take effect until a new animation is set via [#setAnimation(RawAnimation)]
    ...
    /// Sets the controller's [EasingType] override for animations, ignoring the json-based easing preference
    ///
    ...
    /// Mark this controller as _additive_
    ...
    /// Tells the AnimationController that you want to receive the [AnimationStateHandler] while a triggered animation is playing
    ///
    /// This has no effect if no triggered animation has been registered, or one isn't currently playing
    ///
    /// If a triggered animation is playing, it can be checked in your AnimationStateHandler via [#isPlayingTriggeredAnimation()]
    ...
    /// Attempt to trigger an animation from the list of [#triggerableAnimations] this controller contains
    ///
    /// Only animations registered as triggerable via [#triggerableAnim(String, RawAnimation)] can be triggered
    ///
    ...
    /// Set the current position of the [Animation] currently playing
    ///
    ...
    /// Stops and removes a previously [triggered][#triggerAnimation(String)] animation, effectively ending it immediately
    ///
    ...
    /// Every render frame, the `AnimationController` will call this handler for <u>each</u> animatable that is being rendered
    ///
    /// This handler defines which animation should be currently playing, and returning a [PlayState] to tell the controller what to do next
    ///
    /// Example Usage:
    /// <pre>
    /// `AnimationStateHandler myIdleWalkHandler = test ->{if (test.isMoving()){return test.setAndContinue(myWalkAnimation);}else{return test.setAndContinue(myIdleAnimation);}};`</pre>
    ///
    ...
    public interface AnimationStateHandler<A extends GeoAnimatable> {
        ...
        ///     <tr><td>[PlayState#CONTINUE]</td><td>Start or continue animating</td></tr>
        ///     <tr><td>[PlayState#PAUSE]</td><td>Continue animating, freezing at the current animation progress</td></tr>
        ///     <tr><td>[PlayState#STOP]</td><td>Stop animating, resetting for the next animation</td></tr>
        /// </table>
        PlayState handle(AnimationTest<A> animatable);
    }
    /// A handler for keyframe instructions defined in the animation .json
    ///
    /// When the keyframe is encountered, the [KeyframeEventHandler#handle(KeyFrameEvent)] method will be called.
    ...
    /// Check the current state of the controller, including querying the [AnimationStateHandler]
    ///
    ...
    /// Returns whether the given [registered triggerable animation][#triggerableAnim(String, RawAnimation)] is currently triggered
    /// on this controller.
    ///
