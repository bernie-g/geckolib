 ...
 * <p>
 * Note that the timestamp for this keyframe is not contained here, and is instead held by the {@link ActorBoneAnimationEntry#keyframes()} map, if present
 *
 * @param values The actual value for this keyframe. Can technically be null if only {@link #preKeyframe} or {@link #postKeyframe} are present
...
public record ActorBoneAnimationKeyframe(@Nullable ActorBoneAnimationKeyframeValues values, @Nullable String interpolationType, DoubleOrString @Nullable [] easingArgs,
    ...
    /**
     * Parse an ActorBoneAnimationKeyframe instance from raw .json input via {@link Gson}
     */
