...
public class RawGeometryTree
{
	...
	public static RawGeometryTree parseHierarchy(RawGeoModel model, ResourceLocation location)
	{
		...
		while (true)
		{
			...
			{
				RiftLib.LOGGER.warn("Some bones in " + location.toString() + " do not have existing parents: ");
				RiftLib.LOGGER.warn(bones.stream().map(x -> x.getName()).collect(Collectors.joining(", ")));
				break;
			}
