...
public non-sealed interface StatelessGeoSingletonAnimatable extends StatelessAnimatable, SingletonGeoAnimatable {
    ...
    default void playAnimation(RawAnimation animation, Entity relatedEntity, long instanceId) {
        if (relatedEntity.level().isClientSide) {
            ...
            GeckoLibNetwork.send(new StatelessSingletonPlayAnimPacket(GeckoLibNetwork.getSyncedSingletonAnimatableId(this), instanceId, animation), PacketDistributor.TRACKING_ENTITY_AND_SELF.with(() -> relatedEntity));
        }
    }
    ...
    default void stopAnimation(String animation, Entity relatedEntity, long instanceId) {
        if (relatedEntity.level().isClientSide) {
            ...
            GeckoLibNetwork.send(new StatelessSingletonStopAnimPacket(GeckoLibNetwork.getSyncedSingletonAnimatableId(this), instanceId, animation), PacketDistributor.TRACKING_ENTITY_AND_SELF.with(() -> relatedEntity));
        }
    }
