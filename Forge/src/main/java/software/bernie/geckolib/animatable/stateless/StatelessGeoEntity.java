...
public non-sealed interface StatelessGeoEntity extends StatelessAnimatable, GeoEntity {
    ...
    default void playAnimation(RawAnimation animation) {
        ...
        if (self.level().isClientSide) {
            ...
            GeckoLibNetwork.send(new StatelessEntityPlayAnimPacket(self.getId(), false, animation), PacketDistributor.TRACKING_ENTITY_AND_SELF.with(() -> self));
        }
    }
    ...
    default void stopAnimation(String animation) {
        ...
        if (self.level().isClientSide) {
            ...
            GeckoLibNetwork.send(new StatelessEntityStopAnimPacket(self.getId(), false, animation), PacketDistributor.TRACKING_ENTITY_AND_SELF.with(() -> self));
        }
    }
}
