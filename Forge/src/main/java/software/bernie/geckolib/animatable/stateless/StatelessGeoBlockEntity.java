...
public non-sealed interface StatelessGeoBlockEntity extends StatelessAnimatable, GeoBlockEntity {
    ...
    default void playAnimation(RawAnimation animation) {
        ...
        if (self.getLevel() instanceof ServerLevel level) {
            GeckoLibNetwork.send(new StatelessBlockEntityPlayAnimPacket(self.getBlockPos(), animation), PacketDistributor.TRACKING_CHUNK.with(() -> level.getChunkAt(self.getBlockPos())));
        }
        else {
    ...
    default void stopAnimation(String animation) {
        ...
        if (self.getLevel() instanceof ServerLevel level) {
            GeckoLibNetwork.send(new StatelessBlockEntityStopAnimPacket(self.getBlockPos(), animation), PacketDistributor.TRACKING_CHUNK.with(() -> level.getChunkAt(self.getBlockPos())));
        }
        else {
